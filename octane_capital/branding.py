"""Console branding utilities for Octane Capital Lab."""
...
def print_header() -> None:
    """Print the Octane Capital Lab header."""
    print("\n" + "=" * 80)
    print(f"{CHROME}{'OCTANE CAPITAL LAB':^80}{Style.RESET_ALL}")
    print(f"{OBSIDIAN}{'Research Engine':^80}{Style.RESET_ALL}")
    print("=" * 80 + "\n")
...
def print_agent_status(agent_name: str, status: str, details: str = "") -> None:
...
def print_ticker_card(ticker: str, confidence: float, summary: str) -> None:
    """Print a formatted ticker card."""
    print(f"\n{CHROME}{'-' * 80}{Style.RESET_ALL}")
    print(f"{CHROME}  TICKER:{Style.RESET_ALL} {ACCENT}{ticker:<10}{Style.RESET_ALL}", end="")
    print(
        f"{CHROME}CONFIDENCE:{Style.RESET_ALL} "
        f"{SUCCESS if confidence >= 8.0 else WARNING}{confidence:.1f}/10.0{Style.RESET_ALL}"
    )
...
def print_footer() -> None:
    """Print the footer."""
    print(f"\n{CHROME}{'-' * 80}{Style.RESET_ALL}")
    print(f"{OBSIDIAN}{'Research mode only - no live trading':^80}{Style.RESET_ALL}")
    print(f"{CHROME}{'-' * 80}{Style.RESET_ALL}\n")
