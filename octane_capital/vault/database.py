...
Persistent storage using Supabase. Runs in standby mode when no Supabase
...
from ..models import AuditReport, ScoutSignal
...
class Vault:
    ...
    def save_ticker_report(
        ...
        """Save a ticker report and audit to the database.

        Supports both Pydantic models and dict inputs. Returns ``True`` on
        success, ``False`` otherwise.
        """
        if not self.initialized:
            # In ghost mode or without DB, treat as a no-op success
        ...
        try:
            ...
            if audit_report:
                ...
                investment_thesis = audit_data.get("final_assessment", {}).get(
                    "investment_thesis"
                )
            else:
            ...
            record = {
                ...
                "revised_confidence": float(revised_confidence)
