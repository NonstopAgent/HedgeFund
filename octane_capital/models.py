"""Pydantic data models for research outputs."""
...
class ScoutSignal(BaseModel):
    ...
    sentiment_score: float = Field(
        ..., ge=0.0, le=10.0, description="Sentiment/confidence score (0-10)"
    )
    signals: list[str] = Field(
        default_factory=list, description="List of alpha signals detected"
    )
    signal_sources: list[str] = Field(
        default_factory=list, description="Sources of signals"
    )
    summary: str = Field(
        ..., description="Brief summary of why this ticker has alpha potential"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "summary": (
                    "Strong developer community growth and IP expansion indicate "
...
class MoatAnalysis(BaseModel):
    ...
    findings: list[str] = Field(
        default_factory=list, description="Key findings about technical moats"
    )
    patents_mentioned: list[str] = Field(
        default_factory=list, description="Relevant patents identified"
    )
...
class SECAnalysis(BaseModel):
    ...
    revenue_trend: str = Field(..., description="Revenue trend")
    key_metrics: dict[str, Any] = Field(
        default_factory=dict, description="Key financial metrics"
    )
...
class InsiderTrading(BaseModel):
    ...
    notable_transactions: list[str] = Field(
        default_factory=list, description="Notable insider transactions"
    )
...
class FinancialHealth(BaseModel):
    ...
    debt_equity_ratio: float | None = Field(None, description="Debt-to-equity ratio")
...
class FinalAssessment(BaseModel):
    ...
    conviction_score: float = Field(
        ..., ge=0.0, le=10.0, description="Final conviction score (0-10)"
    )
    risk_level: str = Field(..., description="Risk level: Low/Medium/High")
    investment_thesis: str = Field(..., description="Detailed investment thesis")
    recommended_position: str = Field(
        ..., description="Recommended position: Large/Medium/Small/None"
    )
...
class AuditReport(BaseModel):
    ...
    financial_health: FinancialHealth = Field(..., description="Financial analysis")
    final_assessment: FinalAssessment = Field(
        ..., description="Final investment assessment"
    )
        ...
        json_schema_extra = {
            "example": {
                ...
                "final_assessment": {
                    ...
                    "risk_level": "Medium",
                    "investment_thesis": (
                        "Strong buy with exceptional moat and growth trajectory"
                    ),
