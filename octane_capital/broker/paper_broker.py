"""
Octane Capital Lab - Paper Broker
Simulates cash, positions, and fills without live orders.
"""
...
class PaperBroker:
    """Simulated broker with local JSON persistence."""

    def __init__(self, state_path: Optional[Path] = None, prices: Optional[Dict[str, float]] = None):
    ...
    def _load_state(self) -> dict:
        ...
        return {
            "cash": config.STARTING_PAPER_CASH,
            "positions": {},
            "orders": [],
        }
    ...
    def _get_price(self, ticker: str) -> float:
        return self.prices.get(ticker.upper(), 100.0)
    ...
    def preview_order(self, order: OrderRequest) -> OrderPreview:
        ...
        if order.notional and not order.quantity:
    ...
    def place_order(self, order: OrderRequest) -> TradeResult:
        ...
        if order.action == TradeAction.BUY:
            ...
            if total_qty > 0:
                avg_cost = (
                    ...
                    + preview.estimated_quantity * price
                ) / total_qty
            else:
                avg_cost = price
        ...
        result = TradeResult(
            ...
            filled_quantity=preview.estimated_quantity,
    ...
    def portfolio_context(self) -> "PortfolioContext":
        from octane_capital.models import PortfolioContext
