...
from models import ScoutSignal, AuditReport


class Vault:
    """The Vault - Database storage agent"""
    ...
    def save_ticker_report(
        ...
        Supports both Pydantic models and dict inputs
        
        Args:
            ticker_data: Scout ticker data (dict)
            audit_data: Optional auditor data (dict)
            scout_signal: Optional ScoutSignal Pydantic model
            audit_report: Optional AuditReport Pydantic model
            
        Returns:
            True if successful, False otherwise
        """
        if not self.initialized:
            # In ghost mode or without DB, just log
        ...
        try:
            ...
            if scout_signal:
                ...
                scout_confidence = ticker_data.get("confidence_score") or ticker_data.get("sentiment_score", 0.0)
            ...
            if audit_report:
                ...
                revised_confidence = audit_data.get("final_assessment", {}).get("revised_confidence") or audit_data.get("final_assessment", {}).get("conviction_score")
            ...
            # Insert into 'ticker_reports' table
    ...
    def get_recent_tickers(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.client.table("ticker_reports")\
                .select("*")\
