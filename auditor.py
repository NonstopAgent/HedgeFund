"""
Octane Global Trust - The Auditor Agent (CrewAI)
...
from models import AuditReport, MoatAnalysis, SECAnalysis, InsiderTrading, FinancialHealth, FinalAssessment
...
def create_auditor_agent() -> Agent:
    ...
    auditor = Agent(
        ...
        backstory="""You are The Auditor, a senior investment analyst at Octane Global Trust specializing in deep-dive due diligence.
        With expertise in:
...
def parse_audit_output(output: str, ticker: str) -> Optional[AuditReport]:
    ...
    try:
        ...
        if not parsed:
            # Try parsing the output string directly
        ...
        if "revised_confidence" in parsed.get("final_assessment", {}):
            parsed["final_assessment"]["conviction_score"] = parsed["final_assessment"].pop("revised_confidence")
        ...
        try:
            ...
            # Fallback: try to build with minimal required fields
            print_agent_status("THE AUDITOR", "WARNING", f"Partial parse: {str(e)}")
            
            # Create minimal valid AuditReport
            ...
            audit_report = AuditReport(
                ...
                final_assessment=FinalAssessment(
                    conviction_score=float(final_assessment_data.get("conviction_score") or final_assessment_data.get("revised_confidence", 7.0)),
